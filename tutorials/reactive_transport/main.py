...
if (os.name == 'posix'):
    ...
    msh_lib_path = 'C:/Users/group/Source/lib/meshpy'
...
OGS_Config              = {'task_root' : ogs_lib_path + '/tutorials/reactive_transport/project',
                           'task_id'   : 'fm',
                           'dim_no'    : dim_no}
...
indicator_srf = Transmissivity( **IndicatorSRFConfig )
indicator_srf.set_mesh('unstructured', msh.centroid_array[:,0], msh.centroid_array[:,1])
indicator_srf.gen_srf()
offset = np.mean(indicator_srf.data)
#print(np.sqrt(2)*erfinv(1 - 2*0.2))
p = 1.0
mu = np.sqrt(2)*erfinv(1 - 2*p)
indicator = np.sign( indicator_srf.data - offset + mu )

indicator_01 = np.where( indicator == -1.0)
indicator_02 = np.where( indicator ==  1.0)

print( float(len(indicator_01[0])) /( float(len(indicator_01[0])) + float(len(indicator_02[0])) ) )

srf_01 = Transmissivity( **SRF_1_Config )
srf_01.set_mesh('unstructured', msh.centroid_array[:,0], msh.centroid_array[:,1])
srf_01.gen_srf()

srf_02 = Transmissivity( **SRF_2_Config )
srf_02.set_mesh('unstructured', msh.centroid_array[:,0], msh.centroid_array[:,1])
srf_02.gen_srf()

srf_02.data[indicator_01] = srf_01.data[indicator_01]
...
#ogs.plot_mesh(msh)
#ogs.plot_srf(msh, indicator)
ogs.plot_srf(msh, srf_01.data)
